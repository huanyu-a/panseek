...
const SEARCH_URL = "http://www.yulinshufa.cn/plus/search.php?q=";

export class YulinshufaPlugin extends BaseAsyncPlugin {
  constructor() {
    super("yulinshufa", 3);
  }
  ...
  override async search(keyword: string): Promise<SearchResult[]> {
    ...
    for (const r of results) {
      r.unique_id = r.unique_id.replace('plugin-', 'yulinshufa-');
    }

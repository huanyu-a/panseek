...
const SEARCH_URL = (keyword: string) => `https://www.iyuhuage.fun/search/${keyword}-1-time.html`;

export class YuhuagePlugin extends BaseAsyncPlugin {
  constructor() {
    super("yuhuage", 3);
  }
  ...
  override async search(keyword: string): Promise<SearchResult[]> {
    ...
    for (const r of results) {
      r.unique_id = r.unique_id.replace('plugin-', 'yuhuage-');
    }
